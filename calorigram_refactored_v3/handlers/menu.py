...
async def show_welcome_message_with_data(update: Update, context: ContextTypes.DEFAULT_TYPE, user_data):
    ...
    try:
        ...
        logger.info(f"user_data[17] (target_protein): {user_data[17] if len(user_data) > 17 else 'N/A'}")
        logger.info(f"user_data[18] (target_fat): {user_data[18] if len(user_data) > 18 else 'N/A'}")
        logger.info(f"user_data[19] (target_carbs): {user_data[19] if len(user_data) > 19 else 'N/A'}")
        ...
        target_protein = user_data[17] if len(user_data) > 17 else 0.0
        target_fat = user_data[18] if len(user_data) > 18 else 0.0
        target_carbs = user_data[19] if len(user_data) > 19 else 0.0
