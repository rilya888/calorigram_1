...
def validate_image_file(file_data: bytes) -> bool:
    ...
    try:
        ...
        if len(file_data) > MAX_IMAGE_SIZE:  # 20MB лимит
            return False
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            ...
            async with session.get(url) as response:
                ...
                if content_length and int(content_length) > 20 * 1024 * 1024:  # 20MB лимит
                    await processing_msg.edit_text(
                        "❌ **Файл слишком большой**\n\n"
                        "Размер фотографии превышает 20MB. Пожалуйста, отправьте фото меньшего размера.",
                        reply_markup=get_main_menu_keyboard(),
                        parse_mode='Markdown'
                    )
                    return
                ...
                if len(image_content) > 20 * 1024 * 1024:  # 20MB лимит
                    await processing_msg.edit_text(
                        "❌ **Файл слишком большой**\n\n"
                        "Размер фотографии превышает 20MB. Пожалуйста, отправьте фото меньшего размера.",
                        reply_markup=get_main_menu_keyboard(),
