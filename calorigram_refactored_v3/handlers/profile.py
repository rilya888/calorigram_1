...
async def profile_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Получаем часовой пояс из правильного поля (user_data[14])
        timezone = user_data[14] if len(user_data) > 14 else 'UTC'
        ...
        # Получаем БЖУ из базы данных
        target_protein = user_data[17] if len(user_data) > 17 and user_data[17] else 0.0
        target_fat = user_data[18] if len(user_data) > 18 and user_data[18] else 0.0
        target_carbs = user_data[19] if len(user_data) > 19 and user_data[19] else 0.0
...
{subscription_text}
        ...
        # Создаем клавиатуру профиля с кнопкой статистики
...
async def show_meal_statistics(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        from database import get_daily_macros, get_meals_by_type, get_user_target_macros
        ...
        meals_data = get_meals_by_type(user.id)
