...
def create_database() -> bool:
    ...
    try:
        ...
        logger.error(f"SQLite error creating database: {e}")
        return False
...
def get_db_connection():
    ...
    try:
        ...
        logger.error(f"File system error in database connection: {e}")
        if conn:
            conn.rollback()
        raise
    except Exception as e:
...
def get_user_by_telegram_id(telegram_id: int) -> Optional[Tuple[Any, ...]]:
    ...
    try:
        ...
        logger.error(f"Database error getting user by telegram_id {telegram_id}: {e}")
        return None
    except Exception as e:
        logger.error(f"Unexpected error getting user by telegram_id {telegram_id}: {e}")
        return None
...
def get_all_users_for_broadcast() -> List[Tuple[int, str]]:
    ...
    try:
        ...
        logger.error(f"Database error getting all users for broadcast: {e}")
        return []
    except Exception as e:
        logger.error(f"Unexpected error getting all users for broadcast: {e}")
        return []
...
def get_daily_calories(telegram_id: int, date: Optional[str] = None) -> Dict[str, int]:
...
def get_meal_statistics(telegram_id: int, days: int = 7) -> List[Dict[str, Any]]:
...
def get_daily_meals_by_type(telegram_id: int, date: Optional[str] = None) -> Dict[str, Dict[str, Any]]:
...
def get_weekly_meals_by_type(telegram_id: int) -> Dict[str, int]:
...
def get_recent_meals(limit: int = 10) -> List[Tuple[Any, ...]]:
...
def check_user_subscription(telegram_id: int) -> Dict[str, Any]:
